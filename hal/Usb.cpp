...
Return<bool> Usb::enableUsbDataSignal(bool enable) {
  ...
  std::string devicePath = SOC_PATH + deviceName + "/";
  std::string gadgetName = GetProperty(USB_CONTROLLER_PROP, "a600000.dwc3");

  if (enable) {
    if (!WriteStringToFile("1", devicePath + USB_DATA_PATH)) {
    ...
    if (!WriteStringToFile("1", devicePath + ID_PATH)) {
      ALOGE("Not able to turn off host mode");
      result = false;
    }

    if (!WriteStringToFile("0", devicePath + VBUS_PATH)) {
      ALOGE("Not able to set Vbus state");
      result = false;
    }

    if (!WriteStringToFile("0", devicePath + USB_DATA_PATH)) {
